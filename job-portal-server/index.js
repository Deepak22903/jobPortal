  ...
  try {
    ...
    app.post("/post-job", async (req, res) => {
      ...
      if (result.insertedId) {
        return res
          .status(201)
          .json({ message: "Job posted successfully!", id: result.insertedId });
      } else {
    ...
    app.get("/myJobs/:email", async (req, res) => {
      try {
        ...
        console.log("Looking for jobs with postedBy:", decodedEmail);
        ...
        console.error("Detailed error in /myJobs/:email:", err);
    ...
    app.post("/job/:id", async (req, res) => {
      ...
      try {
        const result = await jobsCollections.insertOne({ jobId, resumeLink });
        if (result.insertedId) {
          res
            .status(201)
